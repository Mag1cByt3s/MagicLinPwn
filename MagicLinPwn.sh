...
search_sensitive_content() {
    ...
    sensitive_found=0
    # Find .cnf, .conf, .config, .php and .xml files and search for sensitive content
    find / ! -path "*/proc/*" \( -name "*.cnf" -o -name "*.conf" -o -name "*.config" -o -name "*.php" -o -name "*.xml" \) 2>/dev/null | grep -v "doc\|lib" | while read -r file; do
